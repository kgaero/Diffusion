# -*- coding: utf-8 -*-
"""Copy of Diffusion_lab_one.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/17dmRNikUyZ-0wW9YZyMQ-VFvI85pxva4
...
class EulerMaruyamaSimulator(Simulator):
    ...
    def step(self, xt: torch.Tensor, t: torch.Tensor, h: torch.Tensor):
        ...
        return xt + self.sde.drift_coefficient(xt, t) * h + self.sde.diffusion_coefficient(xt, t) * torch.sqrt(h) * zt
...
**Your answer**: When $\sigma$ is very large, the amplitude of the noise will be large with large dispersion. Trajectories go farther away from zero and look lot more random and spread over time is larger.

When $\sigma$ is close to zero, the aimplitude is smorter and smaller dispersion. The trajectories stay close to X=0 line.

**Your job**: Fill in the `drift_coefficient` and `diffusion_coefficient` methods of the `BrownianMotion` class below.
"""
...
**Your answer**: The scale of Xt becomes large although shape of plot remains similar upto scaling. With smaller sigma, spread closer to 0 and with larger sigma, spread farther away from 0.
...
**Your answer**: When sigma increases the fluctations increase in width. Whereas when Theta increase the convergence towards 0 becomes quicker.
"""
...
**Your answer**: Sigma variation dominates more than Theta proportionally.
...
class LangevinSDE(SDE):
    ...
    def drift_coefficient(self, xt: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        return torch.tensor(0.5) * torch.square(torch.tensor(self.sigma)) *  self.density.score(xt)
